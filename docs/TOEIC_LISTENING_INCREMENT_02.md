...
Les quatre scripts sont versionnés dans `src/content/toeic_listening_conversations_talks.py`. Les quatre WAV privés sont stockés dans `src/content/listening_audio/` et décrits dans `src/content/listening_conversations_talks_assets.json`, avec leur identifiant de stimulus, version de script, hachage SHA-256, durée mesurée, format et origine vocale synthétique générique. Ils ne résident pas sous `public/` et sont explicitement inclus dans la fonction Python Vercel, non dans le bundle statique. Les fichiers sont tous des WAV PCM mono 16 bits à 24 kHz. Aucun actif n’imite ni ne clone une personne identifiable.

## Modèle de lecture contrôlée

La table `diagnostic_stimulus_playback` est la source de vérité pour l’événement de lecture. Elle impose une contrainte unique sur `(attempt_id, stimulus_id)`. À l’action explicite de l’utilisateur, le client demande d’abord l’autorisation de lecture ; le serveur crée l’enregistrement puis retourne une URL audio liée à la tentative. La route audio exige le JWT, la propriété de la tentative et cet enregistrement préexistant ; elle répond avec `Cache-Control: no-store`. Le client demande ensuite la soumission des réponses sans envoyer de compteur, d’identifiant audio, de version de script, de transcript, de choix ou de correction. Le serveur copie les métadonnées d’audit dans les réponses créées.
...
Les routes de catalogue et de démarrage reposent sur des listes blanches de propriétés non sensibles. Elles ne contiennent ni URL publique de fichier, ni nom de fichier privé, ni script, ni choix textuel. La route de revue `GET /api/diagnostic/attempts/<id>/listening-review` exige un JWT, la propriété de la tentative et l’état `completed`. Elle est la seule route qui retourne les scripts attribués aux locuteurs, les choix textuels, les corrections et les explications. Toutes les réponses sensibles et tous les flux audio utilisent `Cache-Control: no-store`.
