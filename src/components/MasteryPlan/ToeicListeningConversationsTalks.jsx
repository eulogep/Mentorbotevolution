...
const ToeicListeningConversationsTalks = ({ onRemediationCreated }) => {
  ...
  const currentAnswersComplete = currentQuestions.length === 2 && currentQuestions.every((item) => answers[item.id]?.selected_index !== undefined);
    ...
    const loadInitialData = async () => {
      try {
        ...
        const [subjectResponse, catalogResponse] = await Promise.all([
          ...
          axios.get('/api/diagnostic/toeic-listening-conversations-talks', authConfig),
        ]);
        if (ignore) return;
  ...
  useEffect(() => {
    ...
    const handleEnded = () => {
      setIsPlaying(false);
      setAudioState('Écoute terminée. Répondez maintenant aux deux questions associées.');
    };
    const handleError = () => {
      setIsPlaying(false);
  ...
  if (!attempt) {
    return (
      <div className="space-y-5">
        {error && <div className="rounded-xl border border-rose-200 bg-rose-50 p-3 text-sm text-rose-800">Erreur : {error}</div>}
        <Card className="border-slate-200 shadow-sm">
          <CardHeader><CardTitle className="flex items-center gap-2 text-lg"><Headphones className="h-5 w-5 text-indigo-700" />Listening — conversations et présentations</CardTitle><CardDescription>Deux conversations et deux présentations originales. Une seule écoute est autorisée par extrait avant la correction différée. Cette activité n’est pas un test TOEIC officiel.</CardDescription></CardHeader>
          <CardContent className="space-y-4">
            {languageSubjects.length ? <><div><label htmlFor="shared-listening-subject" className="block text-sm font-semibold text-slate-800">Parcours de langue associé</label><select id="shared-listening-subject" value={subjectId} onChange={(event) => setSubjectId(event.target.value)} className="mt-2 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-600">{languageSubjects.map((subject) => <option key={subject.id} value={subject.id}>{subject.name}</option>)}</select></div>{listeningReady ? <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-950"><p className="font-semibold">Module prêt</p><p className="mt-1">Les quatre stimuli audio sont disponibles ; leur transcription restera masquée jusqu’à la soumission.</p></div> : <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-950"><p className="font-semibold">Publication audio en cours</p><p className="mt-1">{unavailableAssets.length || 4} stimulus(s) audio sont indisponibles. Le diagnostic reste fermé jusqu’à vérification complète.</p></div>}<Button onClick={startDiagnostic} disabled={starting || !listeningReady} className="w-full bg-indigo-700 hover:bg-indigo-800">{starting ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Préparation…</> : <><ArrowRight className="mr-2 h-4 w-4" />Démarrer le diagnostic d’écoute avancé</>}</Button></> : <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-5 text-sm text-slate-700">Créez d’abord un parcours du domaine Langues pour rattacher vos résultats et vos remédiations à un objectif explicite.</div>}
          </CardContent>
        </Card>
      </div>
    );
  ...
  return (
    <div className="space-y-5">
      ...
      <Card className="border-slate-200 shadow-sm">
        <CardHeader>
          <div className="flex flex-col justify-between gap-2 sm:flex-row sm:items-start"><div><CardTitle className="text-lg">Diagnostic écoute avancé</CardTitle><CardDescription>{diagnostic?.title} · Extrait {stimulusIndex + 1} sur {stimuli.length} · deux questions</CardDescription></div><Badge variant="outline">{currentStimulus?.task_type === 'listening_conversation' ? 'Conversation' : 'Présentation'}</Badge></div>
