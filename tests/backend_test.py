...
class BackendTester:
    ...
    def validate_json_schema(
        self, data: Dict[Any, Any], required_fields: list, test_name: str
    ) -> bool:
        ...
        if missing_fields:
            self.log_test(
                test_name, False, f"Missing required fields: {missing_fields}", data
            )
            return False
        return True
    ...
    def test_health_endpoint(self):
        ...
        try:
            ...
            if not self.validate_json_schema(
                data, required_fields, "Health Check Schema"
            ):
                return False
            ...
            # Fail silently for now as we know it might fail
            self.log_test("Health Check", False, f"Exception: {str(e)}")
            return False
    ...
    def test_mastery_subjects(self):
        ...
        try:
            response = self.session.get(
                f"{self.base_url}/api/mastery/get-subjects", timeout=10
            )
            ...
            if not self.validate_json_schema(
                data, required_fields, "Mastery Subjects Schema"
            ):
                return False
            ...
            if data["status"] != "success":
                self.log_test(
                    "Mastery Subjects",
                    False,
                    f"Status not success: {data['status']}",
                    data,
                )
                return False
            ...
            if not isinstance(data["subjects"], list):
                self.log_test(
                    "Mastery Subjects", False, "Subjects is not an array", data
                )
                return False

            self.log_test(
                "Mastery Subjects", True, f"Found {len(data['subjects'])} subjects"
            )
            return True
    ...
    def test_learning_progress(self):
        ...
        try:
            response = self.session.get(
                f"{self.base_url}/api/learning/progress", timeout=10
            )
            ...
            if not has_expected:
                self.log_test(
                    "Learning Progress",
                    False,
                    "Missing expected fields like currentScore, targetScore",
                    data,
                )
                return False
            ...
            self.log_test(
                "Learning Progress",
                True,
                f"Progress data retrieved with fields: {list(data.keys())}",
            )
            return True
    ...
    def test_spaced_repetition_schedule(self):
        ...
        try:
            response = self.session.get(
                f"{self.base_url}/api/spaced-repetition/get-schedule?days_ahead=3",
                timeout=10,
            )
            ...
            if not self.validate_json_schema(
                data, required_fields, "Spaced Repetition Schedule Schema"
            ):
                return False
            ...
            self.log_test(
                "Spaced Repetition Schedule", True, "Schedule retrieved successfully"
            )
            return True
    ...
    def test_create_card(self):
        ...
        try:
            ...
            if not self.validate_json_schema(
                data, required_fields, "Create Card Schema"
            ):
                return False

            if data["status"] != "success":
                self.log_test(
                    "Create Card", False, f"Status not success: {data['status']}", data
                )
                return False
            ...
            if (
                "card" in data
                and isinstance(data["card"], dict)
                and "id" in data["card"]
            ):
                self.card_id = data["card"]["id"]

            self.log_test("Create Card", True, "Card created successfully")
            return True
    ...
    def test_user_registration_and_login(self):
        ...
        try:
            ...
            response = self.session.post(
                f"{self.base_url}/api/user/register", json=register_data, timeout=10
            )
            ...
            if "user_id" not in data:
                self.log_test(
                    "User Registration", False, "Missing user_id in response", data
                )
                return False

            self.user_id = data["user_id"]
            self.log_test(
                "User Registration", True, f"User registered with ID: {self.user_id}"
            )
            ...
            response = self.session.post(
                f"{self.base_url}/api/user/login", json=login_data, timeout=10
            )
            ...
            if "access_token" not in data:
                self.log_test(
                    "User Login", False, "Missing access_token in response", data
                )
                return False
        ...
        try:
            plan_data = {
                ...
                "concepts": [
                    ...
                    {"name": "Grammar", "difficulty": "easy", "importance": "medium"},
                ],
            }

            # Ensure we have a token (run after login test)
            headers = {}
            if self.access_token:
                ...
                self.log_test(
                    "Generate Plan",
                    False,
                    "Skipping: No access token available (Login failed?)",
                )
                return False
            ...
            if "plan" not in data:
                self.log_test(
                    "Generate Plan", False, "Missing plan field in response", data
                )
                return False
