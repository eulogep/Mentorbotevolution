...
from src.content.toeic_listening_multi_speaker import (
    get_toeic_listening_multi_speaker_diagnostic,
)


PROJECT_ROOT = Path(__file__).resolve().parents[1]
MANIFEST_PATH = PROJECT_ROOT / "src" / "content" / "listening_multi_speaker_assets.json"
PRIVATE_AUDIO_DIR = PROJECT_ROOT / "src" / "content" / "listening_audio"
PUBLIC_AUDIO_DIR = PROJECT_ROOT / "public" / "listening"


def test_multi_speaker_manifest_matches_private_original_audio_assets():
    manifest = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))
    diagnostic = get_toeic_listening_multi_speaker_diagnostic()

    assert manifest["module"] == diagnostic["id"] == "toeic-listening-multi-speaker-v1"
    assert manifest["origin"] == "editorial_original"
    assert manifest["access"] == "private_authenticated_after_server_authorized_playback"
    assert len(manifest["items"]) == 4
    assert len(diagnostic["stimuli"]) == 4
    assert len(diagnostic["items"]) == 12
    ...
    assert not list(PUBLIC_AUDIO_DIR.glob("lms-*.wav"))
